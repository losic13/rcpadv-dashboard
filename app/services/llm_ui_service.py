...
PostgreSQL DB 에서 (day, user_id, use_count) 행을 받아와 페이지가 사용하는
응답 형태로 가공한다. 응답 구조는 login_history 서비스와 동일하다.
...
설계:
  ...
  - 0 사용 일자도 차트가 비지 않도록 days 를 채운다.
  - 개발자 화이트리스트는 login_history 와 공유 (``developer_ids.py``).
"""
...
from app.queries.llm_ui_queries import (
    LLM_UI_SOURCE,
    LLM_UI_USAGE_SQL,
)
from app.repositories import postgres

log = get_logger("service.llm_ui")

# 툴팁에 보여줄 사용자 상위 N명 (login_history 와 동일)
TOOLTIP_TOP_N = 10
...
def fetch_history(start: date, end: date) -> dict[str, Any]:
    """start, end (둘 다 포함) 범위로 LLM UI 사용 이력을 집계한다.
    ...
    log.info(
        "[llm_ui] 조회 시작 start=%s end=%s (SQL: [%s, %s))",
    ...
    log.info(
        "[llm_ui] 원본 행 수: %d (columns=%s)", len(rows), columns,
    )
    ...
    for r in rows:
        ...
        if uid is None:
            uid = ""
        cnt = int(r.get("use_count") or 0)
        if cnt <= 0:
            continue
        by_day_user[day][str(uid)] += cnt

    # 차트 X축: 0 사용 일자도 빈 막대로 표시
    ...
    log.info(
        "[llm_ui] 완료 days=%d, all.total=%d, customer.total=%d, %dms",
        len(days),
